...
class MainWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # self.my_signal.connect(self.showMax)
    ...
    def showMax(self):
        # 修复有问题的最大化
        if not Global.main_widget.isMaximized():
            Global.main_widget.showMaximized()
        else:
            Global.main_widget.showNormal()

    # 最小化
    def showMin(self):
        Global.main_widget.showMinimized()

    # 关闭窗口
    def showClose(self):
        Global.main_widget.close()

    @pyqtSlot()
    def showSkinWidget(self):
        Global.main_widget.skin_widget.show()
    ...
    def showWidget(self):
        Global.main_widget.showNormal()
        Global.main_widget.raise_()
        Global.main_widget.activateWindow()
...
if __name__ == "__main__":
    ...
    translator.load(str("qt_zh_CN.qm"))
    app.installTranslator(translator)

    translator_zh = QTranslator(app)
    # translator_zh.load(QString("360safe_zh.qm"))
    translator_zh.load(str("360safe_zh.qm"))
    app.installTranslator(translator_zh)
    app.setFont(QFont("Arial", 9))'''

    app = QApplication(sys.argv)
    main_widget = MainWidget()
    Global.InitWindows(main_widget)
    Global.main_widget.showWidget()
    sys.exit(app.exec_())
